...
public class QAgent{
	...
	public static void main(String[] args){
		...
		// new QAgent(new GameEnvironment(go, new Go9Bot(go, VariableState.readFromFile(loadFile))), 128);
		new QAgent(new GameEnvironment(game, new RandomPlayer(game)), 1000);
		// new QAgent(new Rabbit(), 16);
	}

	public QAgent(Environment env, int hiddenNodesCount){
		...
		int[] qDimensions = {batchSize, outputSize}; // Q values for each action
		...
		int pQValues = graph.createPlaceholder(qDimensions);
		...
		int error = graph.addOp(new Operations.MatSub(), logits, pQValues);
		int squaredError = graph.addOp(new Operations.TensorSquare(), error);
		int reinforceLoss = graph.addOp(new Operations.TensorAverage(), squaredError);

		int train = graph.trainMinimizer(reinforceLoss, new AdamMinimizerNode());
		// int train = graph.trainMinimizer(reinforceLoss, new AdamMinimizerNode(0.0001));
		
		int[] runRequests = {actionProbs, logits};
